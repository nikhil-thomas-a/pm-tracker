    ...
    <div className="flex-1 overflow-y-auto">
      <div className="max-w-lg mx-auto px-6 py-8">
        <div className="flex items-center justify-between mb-6">
          <h1 className="text-[13px] font-semibold text-pm-text">Settings</h1>
          <button
            onClick={() => dispatch({ type: 'TOGGLE_SETTINGS' })}
            className="text-[11px] text-pm-muted hover:text-pm-text-2"
          >
            ← Back
          </button>
        </div>

        <h2 className="text-[9px] font-medium text-pm-muted uppercase tracking-[0.1em] mb-3">Statuses</h2>
        <p className="text-[10px] text-pm-muted mb-2">Drag to reorder</p>
        ...
        <div className="bg-pm-surface rounded-lg border border-pm-border divide-y divide-pm-border mb-4">
          {displayedStatuses.map(status => (
            ...
            >
              ...
              <span className="text-[10px] text-pm-muted">{status.scope}</span>
              {!status.isDefault && (
                <button
                  onClick={() => {
                  ...
                  className="text-pm-muted hover:text-pm-danger text-[11px]"
                >
                  Delete
                </button>
              )}
              {status.isDefault && <span className="text-pm-dim text-[10px]">🔒</span>}
            </div>
          ))}
        </div>

        <form onSubmit={addStatus} className="bg-pm-surface rounded-lg border border-pm-border p-4">
          <p className="text-[11px] font-medium text-pm-muted mb-3">Add custom status</p>
          <div className="flex gap-2 mb-3">
            <input
              value={label}
              onChange={e => setLabel(e.target.value)}
              placeholder="Status label"
              className="flex-1 text-[11px] bg-pm-surface-up text-pm-text border border-pm-border rounded px-2 py-1.5 outline-none focus:border-pm-accent"
            />
            <input
              type="color"
              value={color}
          ...
          <div className="flex items-center gap-3 mb-3">
            <span className="text-[11px] text-pm-text-2">Scope:</span>
            {(['global', 'project'] as const).map(s => (
              <label key={s} className="flex items-center gap-1 text-[11px] text-pm-text-2 cursor-pointer">
          ...
          <button type="submit" className="bg-pm-accent text-pm-bg text-[11px] font-semibold px-3 py-1.5 rounded transition-colors hover:opacity-90">
            Add status
          </button>
        </form>
      </div>
